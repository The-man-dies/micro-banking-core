...
export default function ClientPage() {
  ...
  const [isFinancialOperationModalOpen, setIsFinancialOperationModalOpen] =
    useState(false);
  const [selectedClientForFinancialOp, setSelectedClientForFinancialOp] =
    useState<Client | null>(null);
  const [financialOperationType, setFinancialOperationType] = useState<
    "deposit" | "payout" | "renew" | null
  >(null);
  ...
  const handleFinancialOperationClick = (
    client: Client,
    type: "deposit" | "payout" | "renew",
  ) => {
  ...
  const handleFinancialOperationSubmit = async (
    clientId: number,
    amount?: number,
  ) => {
    switch (financialOperationType) {
      ...
      case "payout":
        if (amount === undefined)
          throw new Error("Amount is required for payout.");
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-900 min-h-screen">
      ...
      {isFinancialOperationModalOpen &&
        selectedClientForFinancialOp &&
