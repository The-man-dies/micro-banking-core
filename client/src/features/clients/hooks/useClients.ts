...
export const useClients = () => {
  ...
  const updateClient = useCallback(
    async (
      id: number,
      updatedClientData: Partial<
        Omit<Client, "id" | "accountBalance" | "accountExpiresAt" | "status">
      >,
    ) => {
      try {
  ...
  const deposit = useCallback(
    async (clientId: number, amount: number) => {
      try {
  ...
  const payout = useCallback(
    async (clientId: number, amount: number) => {
      try {
        await api(`/clients/${clientId}/payout`, {
          method: "POST",
          data: { amount },
        });
        fetchClients(); // Refresh the list to reflect new balance
  ...
  const expireAccount = useCallback(
    async (clientId: number) => {
      try {
        await api(`/clients/${clientId}/expire`, { method: "POST" });
        fetchClients(); // Refresh the list to reflect new status
      } catch (err: any) {
        throw new Error(err.message || "Failed to expire account.");
      }
    },
    [fetchClients],
  );
