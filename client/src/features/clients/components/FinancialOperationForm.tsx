...
export default function FinancialOperationForm({
  ...
  if (
    ...
    (client.status === "withdraw_only" || client.status === "expired")
  ) {
    formDisabledReason =
      client.status === "withdraw_only"
    ...
    formDisabledReason =
      "Les retraits ne sont pas autorisés pour un compte expiré.";
  }

  const title =
    operationType === "deposit"
      ? `Déposer sur le compte de ${clientLabel}`
      : operationType === "payout"
        ? `Retirer du compte de ${clientLabel}`
    ...
    try {
      if (operationType === "renew") {
        ...
        if (
          ...
          numericAmount !== client.montantEngagement
        ) {
          setError(
            `Le montant du dépôt doit être strictement égal au montant d'engagement (${client.montantEngagement.toLocaleString()} FCFA).`,
          );
          setIsSubmitting(false);
          return;
        }
          ...
          <div className="bg-base-200/50 p-4 rounded-xl space-y-2">
            <p className="text-sm opacity-70">
              ...
              <span className="font-bold opacity-100">
                {clientLabel} (ID: {client.id})
              </span>
            </p>
            <p className="text-sm opacity-70">
              Solde actuel:{" "}
              <span className="font-bold opacity-100">
                {client.accountBalance.toLocaleString()} FCFA
              </span>
            </p>
            <p className="text-sm opacity-70">
              Statut du compte:{" "}
              <span className="font-bold opacity-100">{translatedStatus}</span>
            </p>
          </div>
            ...
            <div className="form-control">
              <label className="label">
                <span className="label-text font-semibold">
                  Montant (
                  {operationType === "renew" ? "Frais de Réactivation" : "FCFA"}
                  ) *
                </span>
              </label>
              <input
