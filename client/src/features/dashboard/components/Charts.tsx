...
import { formateFRDate } from "@/utils/date";
...
export const TransactionTimeSeriesChart: React.FC<{
  ...
  const chartData = {
    labels: data.map((d) => formateFRDate(d.date)),
    datasets: [
      {
        label: "Montant Total",
...
export const CategoryStatsChart: React.FC<{ data: any | undefined }> = ({
  data,
}) => {
  ...
  return (
    <ChartWrapper title="Répartition par Statut de Compte">
      <Doughnut
        ...
        options={{
          ...defaultOptions,
          plugins: { ...defaultOptions.plugins, legend: { position: "top" } },
        }}
      />
    </ChartWrapper>
  );
...
export const AgentDistributionChart: React.FC<{ data: any[] | undefined }> = ({
  data,
}) => {
  if (!data || data.length === 0) {
    return (
...
export const WeeklyAmountChart: React.FC<{ data: any | undefined }> = ({
  data,
}) => {
  if (!data || Object.keys(data).length === 0) {
    return (
  ...
  const chartData = {
    labels: Object.keys(data).map((data) => formateFRDate(data)), // Assuming keys are date strings
    datasets: [
      {
