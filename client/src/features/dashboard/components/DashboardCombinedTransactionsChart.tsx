...
export const DashboardCombinedTransactionsChart: React.FC<
  DashboardCombinedTransactionsChartProps
> = ({ accountingData }) => {
  if (
    !accountingData ||
    Object.keys(accountingData).every(
      (key) => accountingData[key as keyof AccountingData].length === 0,
    )
  ) {
    return (
  ...
  const chartData = {
    labels: allDates.map((date) => formateFRDate(date)),
    datasets: [
      getDataset(
        accountingData.deposits,
        "Dépôts",
        "rgb(34, 197, 94)",
        "rgba(34, 197, 94, 0.1)",
      ),
      getDataset(
        accountingData.payouts,
        "Retraits",
        "rgb(239, 68, 68)",
        "rgba(239, 68, 68, 0.1)",
      ),
      getDataset(
        accountingData.inscriptionFees,
        "Frais d'Inscription",
        "rgb(59, 130, 246)",
        "rgba(59, 130, 246, 0.1)",
      ),
      getDataset(
        accountingData.reactivationFees,
