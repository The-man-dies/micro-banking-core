...
const createAdmin = async () => {
  try {
    ...
    logger.info(
      `process.env.DATABASE_FILE after dotenv.config(): ${process.env.DATABASE_FILE}`,
    );
    ...
    logger.info(
      `Does database file exist before init? ${fs.existsSync(dbFilePath)}`,
    );
    ...
    logger.info(
      `Does database file exist after init? ${fs.existsSync(dbFilePath)}`,
    );
