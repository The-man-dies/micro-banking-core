...
export const checkAccountStatus = async (
  ...
  try {
    ...
    const isDepositOperation =
      req.method === "POST" && req.path.endsWith("/deposit");
    const isPayoutOperation =
      req.method === "POST" && req.path.endsWith("/payout");

    if (client.status === "withdraw_only") {
      if (isDepositOperation) {
        return ApiResponse.error(
          res,
          "Deposits are not allowed in withdraw_only status.",
          null,
          403,
        );
      }
      ...
      if (isDepositOperation || isPayoutOperation) {
        return ApiResponse.error(
          res,
          "Account is expired. No transactions allowed.",
          null,
          403,
        );
      }
    ...
    return ApiResponse.error(
      res,
      "Server error while checking account status",
      null,
      500,
    );
  }
