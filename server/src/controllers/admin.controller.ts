...
const generateTokens = (admin: { id: number; username: string }) => {
  ...
  const accessTokenExpiry = (process.env.JWT_ACCESS_TOKEN_EXPIRES_IN ||
    "15m") as string;
  const refreshTokenExpiry = (process.env.JWT_REFRESH_TOKEN_EXPIRES_IN ||
    "7d") as string;

  const payload = { id: admin.id, username: admin.username };

  // @ts-expect-error :  expiresIn string type causes a TS error
  const accessToken = jwt.sign(payload, accessTokenSecret as jwt.Secret, {
    expiresIn: accessTokenExpiry,
  });
  // @ts-expect-error :  expiresIn string type causes a TS error
  const refreshToken = jwt.sign(payload, refreshTokenSecret as jwt.Secret, {
...
export const status = (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!expiresAt) {
      return ApiResponse.error(
        res,
        "Expiration time not found in token",
        null,
        500,
      );
    }
